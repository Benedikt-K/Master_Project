...
then run spacerPlacer on those clusterings (again i put this into a script)
...
here i only have the scipts left, run these in this order (so one runs twice), since the script depends on the other script (bad programming i know, but it is what it is. also here the distance for identifying the subtype is 10k bp)
