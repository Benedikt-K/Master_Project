...
this should then get us groups of spacer arrays and also a "singleton" folder, where all of the arrays we cant cluster properly are then located. -> cant make a prediction for them
...
here i compare the predictions of both tools. run these in this order, since the script depends on the other script (bad programming i know, but it is what it is.) also here the distance for identifying the subtype is 10k bp. 
--> again both have to be placed in the same folder as test_out for the CasFinder results and out-sp_style_clusters for the EvOr predictions
